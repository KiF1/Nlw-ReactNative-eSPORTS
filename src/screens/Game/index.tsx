...
export function Game() {
  ...
  async function getDiscordUser(adsId: string){
      fetch(`http://10.0.0.109:4000/ads/${adsId}/discord`)
      .then(res => res.json())
      .then(data => setDiscordSelected(data.discord))}

  useEffect(()=>{
    fetch(`http://10.0.0.109:4000/games/${game.id}/ads`).then(res => res.json()).then(data => setDuos(data))},[])
  ...
  return (
    ...
    <SafeAreaView style={styles.container}>
     ...
     <FlatList data={duos} keyExtractor={item => item.id} renderItem={({item})=>(<DuoCard data={item} onConnect={()=> getDiscordUser(item.id)}/>)} 
      horizontal style={styles.containerList} contentContainerStyle={[duos.length > 0 ? styles.contentList : styles.emptyListContent]} showsHorizontalScrollIndicator={false}
